...
void ndepth(std::map<std::string, std::vector<std::string>> network, std::string id, int& depth, int counter = 0){
    for (std::string name : network.at(id)){
        ++ counter;
        if (!network.at(name).empty()){
            ndepth(network, name, depth, counter);
            --counter;
        }
        else{
            if (counter > depth){
